...
const ph = require('../scrape/photooxy.js')
 ...
 const geayubi = fs.readFileSync("./Ariel-SP_IDN/scrape/Result/Asupan/geayubi.json");
 const bocil = fs.readFileSync("./Ariel-SP_IDN/scrape/Result/Asupan/bocil.json");
 const rikagusriani = fs.readFileSync("./Ariel-SP_IDN/scrape/Result/Asupan/rikagusriani.json");
 ...
 const tebakkimia = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebakkimia.json');
 const asahotak = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/asahotak.json');
 const susunkata = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/susunkata.json');
 const tebakkalimat = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebakkalimat.json');
 const tekateki = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tekateki.json');
 const caklontong = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/caklontong.json');
 const tebakbendera = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebakbendera.json');
 const tebakanime = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebakanime.json');
 const tebakkabupaten = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebakkabupaten.json');
 const tebaklagu = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebaklagu.json');
 const tebaklirik = fs.readFileSync('./Ariel-SP_IDN/scrape/Result/Game/tebaklirik.json');
...
//WAKTU
const arieltime = moment().tz('Asia/Jakarta').format('HH:mm:ss')
        if(arieltime < "23:59:00"){
        var arielsptime = 'Selamat Malam 🌗'
}
        if(arieltime < "19:00:00"){
        var arielsptime = 'Selamat Malam 🌖'
}
        if(arieltime < "18:00:00"){
        var arielsptime = 'Selamat Sore 🌇'
}
        if(arieltime < "15:00:00"){
        var arielsptime = 'Selamat Siang 🌞'
}
        if(arieltime < "11:00:00"){
        var arielsptime = 'Selamat Pagi 🌤️'
}
        if(arieltime < "03:00:00"){
        var arielsptime = 'Selamat Dini Hari 🌘'
}
...
const MenuList2 = `Hii *${m.pushName}* ${arielsptime}`
...
if (q.includes('--help')) return reply(examkosong) 
     reply(`*Tunggu Kak ><*`)
const Ariell = fs.readFileSync("./Ariel-SP_IDN/scrape/Result/RandomImage/aesthetic.json");
...
if (q.includes('--help')) return reply(examkosong) 
     reply(`*Tunggu Kak ><*`)
const Ariel1 = fs.readFileSync("./Ariel-SP_IDN/scrape/Result/RandomVideo/storywa.json");
...
Silahkan Klick Button Dibawah Untuk Melihat Beberapa Fitur Yang Tersedia`
  ...
  case 'videomenu':{
 {
  ...
  case 'asumenu':{
 {
                let sections = [ { title: `${m.pushName} ${arielsptime}`, rows: [ { title: "🎥 Asupan Geayoubi", rowId: "#asupan geayubi", description: "Random Video Geayoubi"},{title: "🎥 Asupan Bocil", rowId: "#asupan bocil", description: "Random Video Bocil Sexy"},{title: "🎥 Asupan Rikagusriani", rowId: "#asupan rikagusriani", description: "Random Video Rikagusriani"}, ]}    ]
...
case 'rpgmenu':{
    {
                let sections = [ { title: `${m.pushName} ${arielsptime}`, rows: [ { title: "[ 🏹 ] Berburu", rowId: "#berburu", description: "Berburu Di Hutan"},{title: "[ 🎣 ] Memancing Ikan", rowId: "#mancing", description: "Memancing Ikan"},{title: "[ ⛏️ ] Menambang", rowId: "#menambang", description: "Menambang Untuk Mendapatkan Emas"},{title: "[ ⚒️ ] Mining", rowId: "#mining", description: "Mining Yuk"},{title: "[ ♥️ ] Mengisi Darah", rowId: "#heal", description: "Mengisi Darah Agar Kamu Tidak Mati"}, ]}    ]
...
case 'inikuis':{
	{
                let sections = [ { title: `${m.pushName} ${arielsptime}`, rows: [ { title: "Kuis Math Gampang", rowId: "#kuismath noob", description: "Ez"},{title: "Kuis Match Sedang", rowId: "#kuismath easy", description: "Ea"},{title: "Kuis Math Medium", rowId: "#kuismath medium", description: "Lumayan Broh"},{title: "Kuis Math Hard", rowId: "#kuismath hard", description: "Hm"},{title: "Kuis Math Extreme", rowId: "#kuismath extreme", description: "extreme"},{title: "Kuis Math Deathzone", rowId: "#kuismath deathzone", description: "Males Kasih Judul"}, ]}    ]
...
if (q.includes('--help')) return reply(examplyme) 
   ...
   if (m.isQuotedImage) {
    ...
    fs.writeFileSync(`./${ran}`, buffer)
     ...
     .on("end", () => {
      exec(`webpmux -set exif ./Ariel-SP_IDN/sticker/data.exif ./${ran} -o ./${ran}`, async (error) => {
      sock.sendMessage(
          from, 
          { 
  ...
  if (m.isQuotedVideo) {
   ...
   var rand2 = '777.webp'
	fs.writeFileSync(`./${rand2}`, buffer)
	 ...
	 .on("error", console.error)
	 .on("end", () => {
	 exec(`webpmux -set exif ./Ariel-SP_IDN/sticker/data.exif ./${rand2} -o ./${rand2}`, async (error) => {
	 sock.sendMessage(
	  from, 
	    { 
